...
CREATE TABLE energy_efficiency_ratings (
    ...
    efficiency_rating VARCHAR(10) NOT NULL UNIQUE
);
...
-- Ensure additional_cost_per_km in subscriptions is set from cars
CREATE OR REPLACE FUNCTION set_additional_cost_per_km()
RETURNS TRIGGER AS $$
BEGIN
    NEW.additional_cost_per_km := (SELECT cm.additional_cost_per_km 
                                   FROM car_models cm 
...
CREATE TRIGGER set_additional_cost_per_km_trigger
BEFORE INSERT ON subscriptions
FOR EACH ROW
EXECUTE FUNCTION set_additional_cost_per_km();
...
-- Trigger to update rental phase to 'Delivery' upon reservation
CREATE OR REPLACE FUNCTION update_rental_phase_to_delivery()
RETURNS TRIGGER AS $$
BEGIN
    UPDATE subscriptions SET rental_phase = 'Delivery' WHERE car_id = NEW.car_id AND user_id = NEW.user_id;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER update_rental_phase_trigger
AFTER INSERT ON delivery_status
FOR EACH ROW
EXECUTE FUNCTION update_rental_phase_to_delivery();
